...
function applyFlagEmojiSupportClass() {
...
function shouldShowFlagEmoji() {
  return !document.documentElement.classList.contains("no-flag-emoji");
}
...
function getDisplayTeamLabel(teamName) {
  const team = TEAMS[teamName] || { name: teamName, flag: "" };
  return `${shouldShowFlagEmoji() && team.flag ? `${team.flag} ` : ""}${team.name}`;
}

function renderTeam(teamName) {
  const team = TEAMS[teamName] || { name: teamName, flag: "" };
  const flag = shouldShowFlagEmoji() && team.flag ? `<span class="flag" aria-hidden="true">${team.flag}</span> ` : "";
    ...
    ${predictions.map(prediction => {
      ...
      return `
        <div class="summary-item">
          ...
          <strong>${getDisplayTeamLabel(match.team1)} ${prediction.goals1} - ${prediction.goals2} ${getDisplayTeamLabel(match.team2)}</strong>
